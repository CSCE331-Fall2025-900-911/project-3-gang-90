  ...
  return (
    <main style={{ background: '#e0e0e0', minHeight: '100vh', width: '100vw', overflow: 'hidden', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start' }}>
      <div className="top-bar" style={{ width: '100%', marginBottom: 0 }}>
        <h1 style={{ fontSize: '1.5rem', letterSpacing: 1 }}>Menu Board</h1>
      </div>
      <div style={{
        width: '100vw',
        maxWidth: 1800,
        ...
        {Object.keys(grouped).length === 0 ? (
          <div style={{ textAlign: 'center', fontSize: '1.5rem', color: '#555', marginTop: 80, width: '100%' }}>No items available.</div>
        ) : (
          Object.entries(grouped).map(([category, items]) => (
            <section key={category} style={{
              ...
              <h2 style={{ fontSize: '1rem', color: '#3a3a3a', borderBottom: '2px solid #929292', marginBottom: 8, paddingBottom: 2, width: '100%', textAlign: 'center', letterSpacing: 0.5 }}>{category}</h2>
              <ul style={{ listStyle: 'none', padding: 0, margin: 0, width: '100%' }}>
                {items.map(item => (
                  <li key={item.item_id || item.id} style={{
                    ...
                    <span style={{ color: '#2a7b2a', fontWeight: 800, fontSize: '0.7rem', minWidth: 50, textAlign: 'right' }}>${item.price.toFixed(2)}</span>
                  </li>
                ))}
              </ul>
            </section>
